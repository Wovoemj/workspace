...
import { CONTINENT_OPTIONS, continentForDestination, type ContinentKey } from '@/lib/destinationRegions'

const perPage = 12
const destPerPage = 12

type SortKey = 'recommended' | 'price_asc' | 'price_desc' | 'rating_desc' | 'sales_desc'
type DestSortKey = 'rating_desc' | 'hot_desc'
...
export default function DestinationsPage() {
  ...
  const [loadingProducts, setLoadingProducts] = useState(initialView === 'products')
  ...
  const [destSort, setDestSort] = useState<DestSortKey>('rating_desc')
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      try {
        const res = await fetch(`/api/destinations?per_page=120&page=1`, { cache: 'no-store' })
  ...
  useEffect(() => {
    ...
    async function run() {
      ...
      setSpotsError(null)
      try {
        const all: DestinationRow[] = []
        let pg = 1
        while (pg < 80) {
          const res = await fetch(`/api/destinations?per_page=100&page=${pg}`, { cache: 'no-store' })
          const data = await res.json().catch(() => ({}))
          const list = (data?.destinations ?? data?.items ?? data?.data ?? []) as DestinationRow[]
          all.push(...list)
          if (list.length < 100) break
          pg += 1
        }
        if (!cancelled) setSpotsDest(all)
      } catch (e: any) {
        if (!cancelled) setSpotsError(e?.message || '加载失败')
      } finally {
  ...
  useEffect(() => {
    setDestPage(1)
  }, [continent, destSort, debouncedKeyword])

  const filteredDestinations = useMemo(() => {
    let list = [...spotsDest]
    if (continent !== 'all') {
      list = list.filter((d) => continentForDestination(d) === continent)
    }
    const kw = debouncedKeyword.trim().toLowerCase()
    if (kw) {
      list = list.filter((d) => {
        const hay = `${d.name} ${d.city} ${d.province} ${d.description ?? ''}`.toLowerCase()
        return hay.includes(kw)
      })
    }
    list.sort((a, b) => {
      if (destSort === 'rating_desc') {
    ...
    return list
  }, [spotsDest, continent, debouncedKeyword, destSort])

  const destTotalPages = Math.max(1, Math.ceil(filteredDestinations.length / destPerPage))
  const destPageSlice = filteredDestinations.slice((destPage - 1) * destPerPage, destPage * destPerPage)
  ...
  useEffect(() => {
    ...
    async function run() {
      try {
        ...
        const res = await fetch(`/api/products?${params.toString()}`, { cache: 'no-store', signal: controller.signal })
        const data = await res.json().catch(() => ({}))
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-white via-sky-50 to-white dark:from-slate-950 dark:via-slate-900 dark:to-slate-950">
      <Navbar />
      ...
      <main className="pt-16">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
          <div className="mb-8 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
              <h1 className="text-2xl md:text-3xl font-extrabold text-foreground">探索</h1>
              <p className="text-sm text-muted-foreground mt-1">浏览目的地景点，或筛选旅行产</p>
            </div>
            ...
            <section className="rounded-3xl border border-border/60 bg-card/70 backdrop-blur p-5 shadow-sm">
              <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between mb-6">
                <div className="flex flex-wrap gap-2">
                  {CONTINENT_OPTIONS.map(({ key, label }) => (
                    <button
                      key={key}
                      type="button"
                ...
                <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto md:min-w-[260px]">
                  <div className="relative flex-1">
                    ...
                    <input
                      ...
                      placeholder="搜索景点 / 城市 / 省份"
                      className="input pl-10 w-full"
                    />
                  </div>
                  <select
                    value={destSort}   // value?
                    onChange={(e) => setDestSort(e.target.value as DestSortKey)}
                    className="input w-full sm:w-auto"
                  >
                    <option value="rating_desc">按评分排</option>
                    <option value="hot_desc">按热度排</option>
                  </select>
                </div>
              </div>
              ...
              ) : destPageSlice.length === 0 ? (
                <div className="rounded-2xl border border-border/60 bg-card p-10 text-center text-muted-foreground">
                  暂无符合条件的景点。试试其它大洲或清空搜索?
                </div>
              ) : (
                <>
                  <p className="text-sm text-muted-foreground mb-4">
                    ?{filteredDestinations.length} 个景?
                    {continent !== 'all' ? ` · ${CONTINENT_OPTIONS.find((c) => c.key === continent)?.label}` : ''}
                  </p>
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                    {destPageSlice.map((d) => (
              ...
              <aside className="lg:w-80 shrink-0">
                <div className="rounded-3xl border border-border/60 bg-card/70 backdrop-blur p-5 shadow-sm">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <div>
                      <h2 className="text-xl font-extrabold text-foreground">产品筛</h2>
                  ...
                  <div className="space-y-4">
                    <div>
                      <div className="text-sm font-semibold text-foreground mb-2">关键</div>
                    ...
                    <div>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          <div className="text-xs text-muted-foreground mb-1">最</div>
                          <input
                            inputMode="numeric"
                            type="number"
                  ...
                  <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-4">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground mt-1">
                        {total ? `?${total} ?· ${sortLabel}` : `根据筛选条件实时更?· ${sortLabel}`}
                      </p>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      >
                        ...
                        <option value="sales_desc">销量优</option>
